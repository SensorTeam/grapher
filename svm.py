...
def pca_svm(filename):
	...
	models = (svm.SVC(kernel='linear', C=C),
			  ...
			  svm.SVC(kernel='rbf', gamma=GAMMA, C=0.1))
	...
	cmap_light = ListedColormap(['#AAD5FF', '#FFAAAA', '#AAFFAA'])
	cmap_bold = ListedColormap(['#007FFF', '#FF0000', '#00FF00'])
	...
	for i in range(len(X)):
		...
		for j in range(len(models)):
			...
			if j == 2:
				...
				print("\tEYE %d PREDICTED CLASS: %s"%(i%2+1,p))
	g.close()
